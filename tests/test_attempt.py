...
def test_retryable_dns_connect():
    ...
    assert manager().is_retryable(
        ErrorClass.UPSTREAM_CONNECT_ERROR, first_byte_sent=False) is True
...
def test_retryable_429():
    assert manager().is_retryable(
        ErrorClass.UPSTREAM_RATE_LIMITED, first_byte_sent=False) is True


def test_retryable_5xx():
    assert manager().is_retryable(
        ErrorClass.UPSTREAM_SERVER_ERROR, first_byte_sent=False) is True


def test_retryable_first_token_timeout():
    assert manager().is_retryable(
        ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=False) is True


def test_not_retryable_400():
    """Replaying a malformed request only reproduces the same 400."""
    assert manager().is_retryable(
        ErrorClass.INVALID_CLIENT_REQUEST, first_byte_sent=False) is False


def test_not_retryable_422():
    assert manager().is_retryable(
        ErrorClass.INVALID_TOOL_ARGUMENTS, first_byte_sent=False) is False


def test_not_retryable_client_cancel():
    """Nobody is left to read the retried answer."""
    assert manager().is_retryable(
        ErrorClass.CLIENT_DISCONNECTED, first_byte_sent=False) is False
    assert manager().is_retryable(
        TerminalReason.CANCELLED_BY_CLIENT, first_byte_sent=False) is False


def test_no_retry_after_first_byte_even_if_retryable():
    """Visibility, not the error, decides: a retry would duplicate output."""
    assert manager().is_retryable(
        ErrorClass.UPSTREAM_SERVER_ERROR, first_byte_sent=True) is False
    assert manager().is_retryable(
        ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=True) is False
...
def test_no_retry_after_third_first_byte_failure():
    ...
    assert mgr.should_retry(
        ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=False) == (True, None)
    assert mgr.should_retry(
        ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=False) == (True, None)
    assert mgr.should_retry(ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=False) == (
        False, TerminalReason.RETRY_BUDGET_EXHAUSTED,
    )
...
def test_no_retry_after_delta_sent():
    ...
    mgr = manager()
    assert mgr.should_retry(
        ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True) == (False, None)
...
def test_circuit_open_on_repeated_truncation_same_position():
    ...
    mgr = manager()
    first = mgr.should_retry(
        ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True, position="item_3/args")
    assert first == (False, None)
    assert "item_3/args" not in mgr._circuit_open

    second = mgr.should_retry(
        ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True, position="item_3/args")
...
def test_open_circuit_short_circuits_later_attempts():
    ...
    for _ in range(2):
        mgr.should_retry(
            ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True, position="p")
    assert mgr.should_retry(
        ErrorClass.FIRST_TOKEN_TIMEOUT, first_byte_sent=False, position="p") == (
        False, TerminalReason.RETRY_BUDGET_EXHAUSTED,
    )
...
def test_distinct_positions_do_not_share_a_circuit():
    ...
    mgr = manager()
    assert mgr.should_retry(
        ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True, position="a") == (False, None)
    assert mgr.should_retry(
        ErrorClass.UPSTREAM_TRUNCATED, first_byte_sent=True, position="b") == (False, None)
...
def test_non_whitelisted_error_never_spends_budget():
    ...
    mgr = manager()
    assert mgr.should_retry(
        ErrorClass.INVALID_CLIENT_REQUEST, first_byte_sent=False) == (False, None)
    assert mgr.upstream_switches == 0
    assert mgr.should_retry(
        ErrorClass.UPSTREAM_SERVER_ERROR, first_byte_sent=False) == (True, None)
...
def test_retry_budget_defaults_to_budget_max_upstream_switches():
    ...
    assert narrowed.should_retry(
        ErrorClass.UPSTREAM_SERVER_ERROR, first_byte_sent=False) == (
        False, TerminalReason.RETRY_BUDGET_EXHAUSTED,
    )
...
def test_custom_budget_widens_the_switch_ceiling():
    ...
    for _ in range(4):
        assert mgr.should_retry(
            ErrorClass.UPSTREAM_CONNECT_ERROR, first_byte_sent=False) == (True, None)
